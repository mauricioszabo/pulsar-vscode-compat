...
function convertItem(item, editor, bufferPosition, atomProvider) {
  ...
  const ranges = completionItemRangesToAtomRanges(item);
  ...
  if (ranges && ranges.length) suggestion.ranges = ranges;

  return suggestion;
}

function completionItemRangesToAtomRanges(item) {
  if (!item) return null;

  if (Array.isArray(item.ranges)) {
    const ranges = item.ranges.map(rangeToAtomRange).filter(Boolean);
    return ranges.length ? ranges : null;
  }

  const range = completionItemReplacementRange(item.range);
  const atomRange = rangeToAtomRange(range);
  return atomRange ? [atomRange] : null;
}

function completionItemReplacementRange(range) {
  if (!range) return null;
  if (range.replacing) return range.replacing;
  if (range.inserting) return range.inserting;
  return range;
}
