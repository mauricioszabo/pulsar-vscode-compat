  ...
  const atomProvider = {
    ...
    async getSuggestions({ editor, bufferPosition, triggerCharacter, activatedManually }) {
      ...
      try {
        ...
        const pulsarItems = items.map(item => convertItem(item, editor, bufferPosition, atomProvider))
        const prefix = editor.getTextInBufferRange(getCurrentPrefixRange(editor))
        const candidates = atom.ui.fuzzyMatcher.setCandidates(pulsarItems.map(i => i.text))
...
function convertItem(item, editor, bufferPosition, atomProvider) {
  ...
  const suggestion = {
    ...
    ranges: [completionItemRangesToAtomRanges(item, editor)],
...
function completionItemRangesToAtomRanges(item, editor) {
  if(!item.range) {
    return getCurrentPrefixRange(editor);
