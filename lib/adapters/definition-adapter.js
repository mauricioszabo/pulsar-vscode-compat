...
const { atomPointToPosition, positionToAtomPoint } = require('../utils/position-range');

function positionToSymbolPoint(editor, position) {
  const point = positionToAtomPoint(position);
  if (!point) return null;

  // Symbols View validates positions with `instanceof Point`; array-shaped
  // positions accepted by most Atom editor APIs are not valid symbol-service
  // results. Derive the runtime Point constructor from the editor so this
  // package does not add a dependency on Pulsar's internal `atom` module.
  const currentPoint = editor && typeof editor.getCursorBufferPosition === 'function'
    ? editor.getCursorBufferPosition()
    : null;
  const Point = currentPoint && currentPoint.constructor;
  if (typeof Point === 'function' && Point !== Object) {
      ...
      try {
        ...
        return locations.map(loc => {
          ...
          return {
            ...
            path: uri.fsPath,
            position: positionToSymbolPoint(editor, range.start)
          };
        });
      } catch (e) {
    ...
    async getSymbols(meta) {
      ...
      try {
        ...
        return locations.map(loc => ({
          name: query || editor.getWordUnderCursor(),
          path: loc.uri.fsPath,
          position: positionToAtomPoint(loc.range.start)
        }));
      } catch (e) {
        return null;
      }
  ...
  function flatten(syms, parent) {
    for (const sym of syms) {
      ...
      result.push({
        name: sym.name,
        path: filePath,
        position: positionToAtomPoint(p),
