    ...
    try {
      ...
      const pulsarPkg = {
        ...
        _vscodeExtension: {
          ...
          main: extMain,
          activationEvents,
          activationScopes: getLanguageActivationScopes(extMeta),
          configSections,
        },
      };
...
function buildPulsarActivationMetadata(extMeta) {
  ...
  let hasUnsupportedEvents = false;
  ...
  for (const event of getActivationEvents(extMeta)) {
    ...
    if (languageMatch) {
      ...
      if (!languageId || !addLanguageActivationHooks(hooks, languageId)) hasUnsupportedEvents = true;
      continue;
    }

    const workspaceMatch = event.match(/^workspaceContains:(.+)$/);
    if (workspaceMatch && workspaceMatch[1].trim()) {
      // Pulsar cannot express workspaceContains directly in package metadata.
      // Load only the lightweight generated wrapper after startup; the wrapper
      // checks project roots before it requires the original extension.
      hooks.add('core:loaded-shell-environment');
      continue;
    }

    // Pulsar can defer command and grammar activation natively, and the generated
    // wrapper handles workspaceContains. Other VS Code events such as onView,
    // onUri, onNotebook, onDebug, and onTaskType have no trigger in Pulsar today.
    // Activation events are alternatives, so preserve any triggers we can map instead of
    // letting one unsupported alternative force startup activation. Fall back
    // to startup only when none of the extension's events can be represented.
    hasUnsupportedEvents = true;
  }
  ...
  if (!commandList.length && !hookList.length && hasUnsupportedEvents) {
    return { activationCommands: undefined, activationHooks: undefined };
  }
  return {
...
function getLanguageActivationScopes(extMeta) {
  const scopes = new Set();
  for (const event of getActivationEvents(extMeta)) {
    const match = event.match(/^onLanguage:(.+)$/);
    if (!match || !match[1].trim()) continue;
    const selector = languageToScope(match[1].trim());
    if (!selector || selector === '*') continue;
    for (const scope of selector.split(',').map(value => value.trim()).filter(Boolean)) scopes.add(scope);
  }
...
// Chrome gates SharedArrayBuffer behind cross-origin isolation (COOP/COEP) per
// renderer ExecutionContext; Pulsar's window is not cross-origin isolated, so
// the global is simply absent there. That gate is enforced at the point of
// use, not just as a missing global, so recovering a real constructor from a
// throwaway vm context (a bare V8 context with no attached window) does not
// reliably help: extension bundles observed in practice (e.g. Claude Code's)
// only ever call Atomics.wait on these buffers behind a process.platform ===
// 'win32' guard, and otherwise just do new Int32Array(new
// SharedArrayBuffer(n)) at module load time for bookkeeping. A construct-only
// polyfill (an ArrayBuffer subclass) satisfies that unconditionally, without
// needing real cross-context shared memory. Try to recover a real, usable
// constructor first and fall back to the polyfill so this can't hard-fail.
function _restoreSharedArrayBuffer() {
  if (typeof globalThis.SharedArrayBuffer === 'function') return;

  var SharedArrayBufferConstructor = null;
  try {
    var candidate = require('vm').runInNewContext('SharedArrayBuffer');
    if (typeof candidate === 'function') {
      // Confirm it actually constructs here; a vm-recovered constructor can
      // still be rejected at the point of use by this context's isolation gate.
      new candidate(0);
      SharedArrayBufferConstructor = candidate;
    }
  } catch (e) {}

  if (!SharedArrayBufferConstructor) {
    SharedArrayBufferConstructor = class SharedArrayBuffer extends ArrayBuffer {};
  }
  ...
  roots.forEach(function(root) {
    if (!root || typeof root.SharedArrayBuffer === 'function') return;
    try { root.SharedArrayBuffer = SharedArrayBufferConstructor; } catch (e) {}
  });
}
_restoreSharedArrayBuffer();
...
let _originalActivationPromise = null;
let _activationState = null;
let _activationListeners = [];
...
function activationEvents() {
  return Array.isArray(meta.activationEvents) ? meta.activationEvents : [];
}

function workspaceContainsPatterns() {
  return activationEvents().map(function(event) {
    var match = typeof event === 'string' && event.match(/^workspaceContains:(.+)$/);
...
function matchGlobParts(fileParts, patternParts, fileIndex, patternIndex) {
  if (patternIndex === patternParts.length) return fileIndex === fileParts.length;
  if (patternParts[patternIndex] === '**') {
    return matchGlobParts(fileParts, patternParts, fileIndex, patternIndex + 1) ||
      (fileIndex < fileParts.length && matchGlobParts(fileParts, patternParts, fileIndex + 1, patternIndex));
  }
  return fileIndex < fileParts.length &&
    matchGlobSegment(fileParts[fileIndex], patternParts[patternIndex]) &&
    matchGlobParts(fileParts, patternParts, fileIndex + 1, patternIndex + 1);
}

function projectContainsPattern(projectRoot, pattern) {
  var normalizedPattern = pattern;
  while (normalizedPattern.indexOf('./') === 0) normalizedPattern = normalizedPattern.slice(2);
  while (normalizedPattern.indexOf('/') === 0) normalizedPattern = normalizedPattern.slice(1);
  if (normalizedPattern.indexOf('*') === -1 && normalizedPattern.indexOf('?') === -1) {
    try { return fs.existsSync(path.join(projectRoot, normalizedPattern)); } catch (e) { return false; }
  }

  var patternParts = normalizedPattern.split('/').filter(Boolean);
  function visit(directory, relativeParts) {
    var entries;
    try { entries = fs.readdirSync(directory, { withFileTypes: true }); } catch (e) { return false; }
    for (var index = 0; index < entries.length; index += 1) {
      var entry = entries[index];
      var entryParts = relativeParts.concat(entry.name);
      if (matchGlobParts(entryParts, patternParts, 0, 0)) return true;
      if (entry.isDirectory() && !entry.isSymbolicLink() && visit(path.join(directory, entry.name), entryParts)) return true;
    }
    return false;
  }
...
function workspaceActivationMatches() {
  var patterns = workspaceContainsPatterns();
  if (!patterns.length || !global.atom || !atom.project || typeof atom.project.getPaths !== 'function') return false;
  return atom.project.getPaths().some(function(root) {
    return patterns.some(function(pattern) { return projectContainsPattern(root, pattern); });
  });
}

function languageActivationMatches() {
  var scopes = Array.isArray(meta.activationScopes) ? meta.activationScopes : [];
  if (!scopes.length || !global.atom || !atom.workspace || typeof atom.workspace.getActiveTextEditor !== 'function') return false;
  var editor = atom.workspace.getActiveTextEditor();
  var grammar = editor && typeof editor.getGrammar === 'function' ? editor.getGrammar() : null;
  return !!(grammar && scopes.indexOf(grammar.scopeName) !== -1);
}

function disposeActivationListeners() {
  _activationListeners.splice(0).forEach(function(disposable) {
    try { if (disposable && typeof disposable.dispose === 'function') disposable.dispose(); } catch (e) {}
  });
}

function watchDeferredActivation() {
  if (_activationListeners.length || !global.atom) return;
  if (atom.project && typeof atom.project.onDidChangePaths === 'function') {
    _activationListeners.push(atom.project.onDidChangePaths(function() { maybeActivateExtension(); }));
  }
  if (atom.workspace && typeof atom.workspace.observeActiveTextEditor === 'function') {
    _activationListeners.push(atom.workspace.observeActiveTextEditor(function(editor) {
      maybeActivateExtension();
      if (editor && typeof editor.onDidChangeGrammar === 'function') {
        _activationListeners.push(editor.onDidChangeGrammar(function() { maybeActivateExtension(); }));
      } else if (editor && typeof editor.observeGrammar === 'function') {
        _activationListeners.push(editor.observeGrammar(function() { maybeActivateExtension(); }));
      }
    }));
  }
}

function activateOriginalExtension() {
  return activateCompatPackage().then(vscode => {
    ...
    try {
      _context = new vscode.ExtensionContext(meta.id, extensionPath, 1, _activationState);
    } catch (e) {
...
function maybeActivateExtension() {
  if (_originalActivationPromise) return _originalActivationPromise;
  var hasWorkspaceTrigger = workspaceContainsPatterns().length > 0;
  if (hasWorkspaceTrigger && !workspaceActivationMatches() && !languageActivationMatches()) {
    watchDeferredActivation();
    return Promise.resolve();
  }
  disposeActivationListeners();
  _originalActivationPromise = publishActivationPromise(activateOriginalExtension());
  return _originalActivationPromise;
}
...
module.exports = { installFromOpenVsx, selectOpenVsxDownload, wrapAndInstall, buildAtomConfig, convertSchema, getConfigSections, getActivationEvents, buildPulsarActivationMetadata, getLanguageActivationScopes, generateWrapperMain, generateVSCodeRequireShim };
