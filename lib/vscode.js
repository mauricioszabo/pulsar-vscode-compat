...
const { TabInputText, TabInputNotebook, TabInputTextDiff, TabInputCustom, TabInputWebview, TabInputTerminal } = require('./types/tab-input');
...
class FileSystemError extends Error {
  constructor(messageOrUri, code = 'Unknown') {
    const message = messageOrUri && typeof messageOrUri === 'object' && messageOrUri.fsPath
      ? messageOrUri.fsPath
      : (messageOrUri === undefined ? '' : String(messageOrUri));
    super(message);
...
class NotebookCellOutputItem {
  ...
  static error(value) {
    const error = value || {};
    return new NotebookCellOutputItem(Buffer.from(JSON.stringify({
      name: error.name || 'Error',
      message: error.message || String(value),
      stack: error.stack
    }), 'utf8'), 'application/vnd.code.notebook.error');
  }
}
