...
const { atomPointToPosition, positionToAtomPoint, rangeToAtomRange } = require('../utils/position-range');
...
// Pulsar's TextBuffer has no `changeCount`, so the compat layer keeps its own
// monotonically increasing version per buffer, bumped by workspace.js right
// before it fires onDidChangeTextDocument.
//
// This has to be right: language servers use the document version to invalidate
// their caches. ElixirLS, for example, reuses its cached parse of a file
// whenever the cached version is >= the version on the incoming change, so a
// version that never moves makes every edit after the first didOpen invisible
// to the server — completions then get answered against the on-disk file.
const bufferVersions = new WeakMap();

function documentVersion(buffer) {
  if (!buffer) return 1;
  return bufferVersions.get(buffer) || 1;
}

function bumpDocumentVersion(buffer) {
  if (!buffer) return 1;
  const next = documentVersion(buffer) + 1;
  bufferVersions.set(buffer, next);
  return next;
}
...
class TextDocument {
  ...
  // VSCode TextDocument versions are positive numbers, starting at 1 for a
  // freshly opened document. vscode-languageclient's code2ProtocolConverter also
  // treats a falsy version as "not a document" when it needs to send a full
  // textDocument/didChange notification, so 0 is not a usable starting point.
  get version() {
    return documentVersion(this._editor.getBuffer && this._editor.getBuffer());
  }
  ...
  getText(range) {
    if (range) return this._editor.getTextInBufferRange(rangeToAtomRange(range));
  ...
  offsetAt(position) {
    return this._editor.getBuffer().characterIndexForPosition(positionToAtomPoint(position));
  }

  positionAt(offset) {
    const point = this._editor.getBuffer().positionForCharacterIndex(offset);
    return atomPointToPosition(point);
  }
