...
const { atomRangeToRange, atomPointToPosition, positionToAtomPoint, rangeToAtomRange } = require('../utils/position-range');
  ...
  _apply() {
    this._editor.transact(() => {
      for (const op of this._ops) {
        if (op.type === 'replace') {
          this._editor.setTextInBufferRange(rangeToAtomRange(op.range), op.value);
        } else if (op.type === 'insert') {
          const point = positionToAtomPoint(op.location);
          this._editor.setTextInBufferRange([point, point], op.value);
        } else if (op.type === 'delete') {
          this._editor.setTextInBufferRange(rangeToAtomRange(op.range), '');
        }
      }
    });
  }
...
class TextEditor {
  ...
  set selection(sel) {
    this._editor.setSelectedBufferRange(rangeToAtomRange(sel), { reversed: sel.isReversed });
  }
  ...
  set selections(sels) {
    this._editor.setSelectedBufferRanges(sels.map(rangeToAtomRange));
  }
  ...
  insertSnippet(snippet, location) {
    ...
    if (location) {
      const locationRange = location instanceof Selection
        ? rangeToAtomRange(location)
        : [positionToAtomPoint(location), positionToAtomPoint(location)];
      this._editor.setSelectedBufferRange(locationRange);
    }
  ...
  setDecorations(decorationType, rangesOrOptions) {
    ...
    for (const item of items) {
      ...
      const marker = layer.markBufferRange(rangeToAtomRange(range), { invalidate: 'never' });
  ...
  revealRange(range, revealType) {
    this._editor.scrollToBufferPosition(positionToAtomPoint(range.start));
  }
...
function selectionFromAtomSelection(selection) {
  ...
  return new Selection(
    atomPointToPosition(anchor),
    atomPointToPosition(active)
  );
}

function endPointRange(range) {
  const end = range.end || range.start;
  const point = positionToAtomPoint(end);
  return [point, point];
}
