...
function createTelemetryLogger(sender, options) {
  // Pulsar does not forward VSCode extension telemetry. Keep the complete logger
  // contract so extensions can subscribe safely while reporting both telemetry
  // categories as permanently disabled.
  const onDidChangeEnableStates = new EventEmitter();
  return {
    get isUsageEnabled() { return false; },
    get isErrorsEnabled() { return false; },
    onDidChangeEnableStates: onDidChangeEnableStates.event,
    logUsage() {},
    logError() {},
    dispose() { onDidChangeEnableStates.dispose(); }
  };
}
