...
function executeCommand(command, ...args) {
  ...
  if (command === 'workbench.action.output.show' || command === 'workbench.action.showOutputChannels') {
    require('./window').showOutputChannel(args[0]);
  ...
  if (command === 'python.viewLanguageServerOutput') {
    require('./window').showOutputChannel('Python Language Server');
    return Promise.resolve(undefined);
  }

  if (command === 'python.viewOutput') {
    require('./window').showOutputChannel('Python');
    return Promise.resolve(undefined);
  }
