...
const { atomRangeToRange, atomPointToPosition } = require('../utils/position-range');
const { makeAutocompleteProvider } = require('../adapters/completion-adapter');
const { registerHoverProvider: _registerHover, provideHover: _provideHover } = require('../adapters/hover-adapter');
...
const { registerFindReferencesProvider, provideFindReferences } = require('../service/find-references');
...
async function matchingSymbolProviders(meta) {
  const matches = [];
  for (const provider of symbolProviders.slice()) {
    if (!provider || typeof provider.canProvideSymbols !== 'function') continue;
    try {
      const score = await provider.canProvideSymbols(meta);
      if (score) matches.push({ provider, score: score === true ? 1 : score });
    } catch (e) {
      console.error('[vscode-compat] symbol provider match error:', e);
    }
  }
  return matches;
}

// Pulsar's service hub delivers symbol.provider once, and Symbols View copies
// the returned array into its broker. Keep this object stable so VSCode
// providers registered later (during extension activation) remain visible.
const symbolProviderProxy = {
  name: 'VSCode language providers',
  packageName: 'pulsar-vscode-compat',
  isExclusive: false,

  async canProvideSymbols(meta) {
    const matches = await matchingSymbolProviders(meta);
  ...
  async getSymbols(meta) {
    ...
    await Promise.all(matches.map(async ({ provider }) => {
      try {
        const symbols = await provider.getSymbols(meta);
        if (Array.isArray(symbols)) results.push(...symbols);
      } catch (e) {
        console.error('[vscode-compat] symbol provider error:', e);
      }
    }));
...
function createDiagnosticCollection(name) {
  const collection = new DiagnosticCollection(name || 'vscode-compat', _linterIndie, uris => {
    _onDidChangeDiagnostics.fire({ uris });
  });
  diagnosticCollections.push(collection);
  return collection;
}

function onDidChangeDiagnostics(listener, thisArg, disposables) {
  return _onDidChangeDiagnostics.event(listener, thisArg, disposables);
}
...
async function executeCompletionItemProvider(uri, position, triggerCharacter) {
  ...
  const pos = atomPointToPosition(position);
...
function registerReferencesProvider(documentSelector, provider) {
  ...
  symbolProviders.push(symProvider);
  const unregisterFindReferences = registerFindReferencesProvider(documentSelector, provider);
  return new Disposable(() => {
    const idx = symbolProviders.indexOf(symProvider);
    if (idx >= 0) symbolProviders.splice(idx, 1);
    unregisterFindReferences();
  });
}
...
function registerSignatureHelpProvider(documentSelector, provider, metaOrFirstChar, ...moreChars) {
  ...
  const setupForEditor = (editor) => {
    ...
    const d = editor.onDidInsertText(async ({ text }) => {
      ...
      const pos = atomPointToPosition(editor.getCursorBufferPosition());
...
function registerCodeActionsProvider(documentSelector, provider, metadata) {
  ...
  if (!atom.commands._registered_code_action) {
    ...
    atom.commands.add('atom-text-editor', commandId, async () => {
      ...
      const doc = new TextDocument(editor);
      const sel = atomRangeToRange(editor.getSelectedBufferRange());
...
module.exports = {
  ...
  _symbolProviders: [symbolProviderProxy],
  _registeredSymbolProviders: symbolProviders,
  _symbolProviderProxy: symbolProviderProxy,
