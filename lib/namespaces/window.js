...
const { TabInputText, TabInputWebview } = require('../types/tab-input');
const { Disposable } = require('../types/disposable');
const { CancellationTokenSource } = require('../types/cancellation');
const outlineService = require('../service/outline');
const path = require('path');
const fs = require('fs');
const os = require('os');
const { pathToFileURL } = require('url');
const vscodeThemeCss = require('../utils/vscode-theme-css');
...
const _onDidStartTerminalShellExecution = new EventEmitter();
const _onDidEndTerminalShellExecution = new EventEmitter();
...
const _textEditorWrappers = new WeakMap();
const _terminals = [];
const _terminalItemsByUri = new Map();
const _webviewItemsByUri = new Map();
let _terminalOpenerDisposable = null;
let _webviewOpenerDisposable = null;
...
function _init() {
  ...
  atom.workspace.onDidChangeActiveTextEditor(editor => {
    ...
    _onDidChangeActiveTextEditor.fire(usableEditor ? wrapEditor(usableEditor) : undefined);
  });
  ...
  atom.workspace.observeTextEditors(editor => {
    ...
    const selectionDisposable = observeAtomEditorSelection(editor, () => {
      fireTextEditorSelectionChange(editor);
    });

    editor.onDidDestroy(() => {
      if (selectionDisposable && typeof selectionDisposable.dispose === 'function') selectionDisposable.dispose();
      if (_lastActiveTextEditor === editor) _lastActiveTextEditor = undefined;
      _onDidChangeVisibleTextEditors.fire(atom.workspace.getTextEditors().map(e => new TextEditor(e)));
    });
  });

  try { vscodeThemeCss.applyCurrentHostTheme({ document, window, atom }); } catch (e) {}

  atom.themes.onDidChangeActiveThemes(() => {
    let ColorTheme;
    try { vscodeThemeCss.applyCurrentHostTheme({ document, window, atom }); } catch (e) {}
    try { ColorTheme = require('./window').activeColorTheme; } catch (e) { ColorTheme = { kind: 1, id: 'default' }; }
...
function wrapEditor(atomEditor) {
  ...
  let wrapper = _textEditorWrappers.get(atomEditor);
  if (!wrapper) {
    wrapper = new TextEditor(atomEditor);
    _textEditorWrappers.set(atomEditor, wrapper);
  }
  return wrapper;
}

function observeAtomEditorSelection(editor, callback) {
  if (!editor || typeof callback !== 'function') return null;
  try {
    if (typeof editor.onDidChangeSelectionRange === 'function') {
      return editor.onDidChangeSelectionRange(callback);
    }
  } catch (e) {}
  try {
...
function fireTextEditorSelectionChange(atomEditor) {
  if (!isUsableAtomTextEditor(atomEditor)) return;
  _lastActiveTextEditor = atomEditor;
  const textEditor = wrapEditor(atomEditor);
...
function viewColumnToSplit(viewColumn) {
  if (viewColumn === ViewColumn.Beside) return 'right';
  if (typeof viewColumn === 'number' && viewColumn >= ViewColumn.Two) return 'right';
  return undefined;
}

function viewColumnToOpenOptions(viewColumn, preserveFocus) {
  const activate = !preserveFocus;
  const split = viewColumnToSplit(viewColumn);
  const options = {
    location: 'center',
...
function getWorkspacePanes() {
  try {
    if (atom.workspace && typeof atom.workspace.getPanes === 'function') {
      return atom.workspace.getPanes().filter(Boolean);
    }
  } catch (e) {}
  try {
    const center = atom.workspace && typeof atom.workspace.getCenter === 'function' && atom.workspace.getCenter();
    if (center && typeof center.getPanes === 'function') return center.getPanes().filter(Boolean);
  } catch (e) {}
  try {
    const activePane = atom.workspace && typeof atom.workspace.getActivePane === 'function' && atom.workspace.getActivePane();
...
function paneItems(pane) {
  try {
    if (pane && typeof pane.getItems === 'function') return pane.getItems();
  } catch (e) {}
  try {
    if (pane && Array.isArray(pane.items)) return pane.items;
  } catch (e) {}
  return [];
}

function viewColumnForPane(pane) {
  const panes = getWorkspacePanes();
  const index = panes.indexOf(pane);
  return index >= 0 ? index + 1 : undefined;
}

function viewColumnForItem(item) {
  try {
    const pane = atom.workspace && typeof atom.workspace.paneForItem === 'function' && atom.workspace.paneForItem(item);
    return pane ? viewColumnForPane(pane) : undefined;
  } catch (e) {
    return undefined;
  }
}

function tabInputForItem(item) {
  if (item && item._vscodeCompatWebviewViewType) return new TabInputWebview(item._vscodeCompatWebviewViewType);
  try {
    const uri = item && typeof item.getURI === 'function' && item.getURI();
    if (uri) return new TabInputText(typeof uri === 'string' ? Uri.parse(uri) : uri);
  } catch (e) {}
  return {};
}

function tabForItem(item, pane) {
  return {
    label: item && typeof item.getTitle === 'function' ? item.getTitle() : '',
    input: tabInputForItem(item),
    isActive: pane && typeof pane.getActiveItem === 'function' ? pane.getActiveItem() === item : false,
    isDirty: false,
    isPinned: true,
    isPreview: false
  };
...
function buildTabGroups() {
  ...
  return panes.map((pane, index) => {
    const tabs = paneItems(pane).map(item => tabForItem(item, pane));
    const activeTab = tabs.find(tab => tab.isActive) || tabs[0] || null;
    let isActive = false;
    try { isActive = atom.workspace.getActivePane && atom.workspace.getActivePane() === pane; } catch (e) {}
    return { viewColumn: index + 1, tabs, activeTab, isActive };
  });
}
...
async function showTextDocument(documentOrUri, optionsOrColumn, preserveFocus) {
  ...
  if (atomEditor) {
    const openOptions = viewColumnToOpenOptions(opts.viewColumn, !activate);
...
function findOutputChannelByName(name) {
  if (!name) return null;
  if (outputChannels.has(name)) return outputChannels.get(name);
  const normalized = String(name).toLowerCase();
  for (const [channelName, channel] of outputChannels) {
    if (String(channelName).toLowerCase() === normalized) return channel;
  }
  for (const [channelName, channel] of outputChannels) {
    const candidate = String(channelName).toLowerCase();
    if (candidate.includes(normalized) || normalized.includes(candidate)) return channel;
  }
  return null;
}

function getOutputChannel(name) {
  return findOutputChannelByName(name);
}

function showOutputChannel(name) {
  let channel = findOutputChannelByName(name);
  if (!channel && !name) {
    channel = findOutputChannelByName('Python Language Server')
      || findOutputChannelByName('Language Server')
      || (outputChannels.size === 1 ? Array.from(outputChannels.values())[0] : null);
  }
...
// --- Terminal ---
function registerTerminalLinkProvider(provider) {
  // Pulsar has no terminal link provider service equivalent. Keep the VSCode API
  // surface available so extensions can register best-effort providers without
  // failing activation.
...
function ensureWebviewOpener() {
  if (_webviewOpenerDisposable) return;
  _webviewOpenerDisposable = atom.workspace.addOpener(uri => {
    const value = typeof uri === 'string' ? uri : uri && uri.toString ? uri.toString() : String(uri);
    if (!value.startsWith('pulsar://pulsar-vscode-compat/webview/')) return;
    return _webviewItemsByUri.get(value);
  });
}
...
// --- Webview ---
const WEBVIEW_API_SCRIPT_URL = pathToFileURL(path.join(__dirname, '..', 'webview-api.js')).href;

function extractWebviewNonce(html) {
  const source = String(html || '');
  let match = source.match(/<script\b[^>]*\bnonce=["']([^"']+)["']/i);
  if (match) return match[1];
  match = source.match(/script-src[^;>]*'nonce-([^'";\s>]+)'/i);
  if (match) return match[1];
  return '';
}

function buildWebviewApiScript(nonce) {
  const nonceAttribute = nonce ? ` nonce="${escapeHtml(nonce)}"` : '';
  try {
    // Inline this bootstrap instead of loading it as a file:// script. Chromium
    // may reuse a cached external file across webview reloads, which can leave
    // stale acquireVsCodeApi/EventTarget shims active while testing fixes.
    const source = fs.readFileSync(path.join(__dirname, '..', 'webview-api.js'), 'utf8')
      .replace(/<\/script/gi, '<\\/script');
    return `<script${nonceAttribute}>${source}</script>`;
  } catch (error) {
    return `<script${nonceAttribute} src="${WEBVIEW_API_SCRIPT_URL}?v=${Date.now()}"></script>`;
  }
}

function injectWebviewApiScript(html) {
  const source = String(html || '');
  if (source.includes(WEBVIEW_API_SCRIPT_URL) || source.includes('webview-api.js')) return source;
  const apiScript = buildWebviewApiScript(extractWebviewNonce(source));
  if (/<script\b/i.test(source)) {
    return source.replace(/<script\b/i, `${apiScript}<script`);
  }
  if (/<head\b[^>]*>/i.test(source)) {
    return source.replace(/<head\b[^>]*>/i, match => `${match}${apiScript}`);
  }
  if (/<html\b[^>]*>/i.test(source)) {
    return source.replace(/<html\b[^>]*>/i, match => `${match}<head>${apiScript}</head>`);
  }
  return `${apiScript}${source}`;
}

function ensureCspAllowsDataFonts(csp) {
  if (!csp) return csp;
  if (/\bfont-src\b/i.test(csp)) {
    return csp.replace(/\bfont-src\b([^;]*)/i, (match, sources) => {
      return /(?:^|\s)data:(?:\s|$)/.test(sources) ? match : `${match} data:`;
    });
  }
  const trimmed = csp.trim();
  return `${trimmed}${trimmed.endsWith(';') ? '' : ';'} font-src file: data:;`;
}

function ensureWebviewTitle(html, title) {
  const source = String(html || '');
  if (/<title\b/i.test(source)) return source;
  const titleTag = `<title>${escapeHtml(title || '')}</title>`;
  if (/<head\b[^>]*>/i.test(source)) {
    return source.replace(/<head\b[^>]*>/i, match => `${match}${titleTag}`);
  }
  if (/<html\b[^>]*>/i.test(source)) {
    return source.replace(/<html\b[^>]*>/i, match => `${match}<head>${titleTag}</head>`);
  }
  return `${titleTag}${source}`;
}

function normalizeWebviewHtml(html, title) {
  let source = ensureWebviewTitle(html, title);
  const nonce = extractWebviewNonce(source);
  const themePayload = vscodeThemeCss.buildCurrentThemePayload({ document, window, atom });
  source = vscodeThemeCss.injectThemeIntoHtml(source, {
    css: themePayload.css,
    themeClass: themePayload.themeClass,
    nonce
  });
  source = injectWebviewApiScript(source);
  source = source.replace(
    /(<meta\b[^>]*http-equiv=["']Content-Security-Policy["'][^>]*\bcontent=)(["'])([\s\S]*?)(\2)([^>]*>)/i,
    (match, prefix, quote, csp, closeQuote, suffix) => `${prefix}${quote}${ensureCspAllowsDataFonts(csp)}${closeQuote}${suffix}`
  );
  return source;
}
...
function createWebviewPanel(viewType, title, showOptions, options) {
  ...
  const container = document.createElement('div');
  container.style.cssText = 'position:relative;width:100%;height:100%;min-height:0;display:flex;flex-direction:column;overflow:hidden;';

  const iframe = document.createElement('iframe');
  iframe.style.cssText = 'display:block;flex:1 1 auto;width:100%;height:100%;min-height:0;border:none;';
  // file:// webview documents must keep their same-origin privileges or
  // Chromium refuses to load their local CSS/JS resources. This combination
  // causes a Chromium sandbox warning, but without it the webview cannot boot.
  ...
  const webviewUri = `pulsar://pulsar-vscode-compat/webview/${++_webviewCounter}`;
  const webviewHtmlPath = path.join(os.tmpdir(), 'pulsar-vscode-compat-webviews', `webview-${_webviewCounter}.html`);
  let _html = '';
  let _themeDisposable = null;
  const postThemeUpdate = () => {
    try {
      const payload = vscodeThemeCss.buildCurrentThemePayload({ document, window, atom });
  ...
  if (atom.themes && typeof atom.themes.onDidChangeActiveThemes === 'function') {
    try { _themeDisposable = atom.themes.onDidChangeActiveThemes(postThemeUpdate); } catch (e) {}
  }
  const webview = {
  ...
  Object.defineProperty(webview, 'html', {
    ...
    set(v) {
      _html = v;
      // about:srcdoc inherits Pulsar/Electron's parent CSP, which can block the
      // webview's own inline nonce scripts before extension code starts. Write a
      // temporary file:// document instead: file: frames are allowed, and the
      // document then enforces the extension-provided CSP rather than srcdoc's
      // inherited one.
      fs.mkdirSync(path.dirname(webviewHtmlPath), { recursive: true });
      fs.writeFileSync(webviewHtmlPath, normalizeWebviewHtml(v, title), 'utf8');
      iframe.removeAttribute && iframe.removeAttribute('srcdoc');
      iframe.src = pathToFileURL(webviewHtmlPath).href;
    }
  });
  ...
  const paneItem = {
    uri: webviewUri,
    getURI() { return webviewUri; },
    getTitle() { return title; },
    getElement() { return container; },
    getDefaultLocation() { return 'center'; },
    getAllowedLocations() { return ['center', 'left', 'right', 'bottom']; },
    _vscodeCompatWebviewViewType: viewType,
    isPermanentDockItem() { return false; },
    onDidDestroy(cb) { return _onDidDispose.event(cb); },
    destroy() {
      _webviewItemsByUri.delete(webviewUri);
      try { if (_themeDisposable && typeof _themeDisposable.dispose === 'function') _themeDisposable.dispose(); } catch (e) {}
      try { fs.unlinkSync(webviewHtmlPath); } catch (e) {}
      _onDidDispose.fire();
    }
  };

  _webviewItemsByUri.set(webviewUri, paneItem);
  atom.workspace.open(webviewUri, viewColumnToOpenOptions(column, preserveFocus));
  ...
  return {
    ...
    get viewColumn() { return viewColumnForItem(paneItem) || (typeof column === 'number' && column > 0 ? column : ViewColumn.One); },
    iconPath: undefined,
    reveal(viewColumn, pf) {
      const preserve = typeof pf === 'boolean' ? pf : false;
      const targetColumn = viewColumn === undefined ? ViewColumn.Active : viewColumn;
      atom.workspace.open(webviewUri, viewColumnToOpenOptions(targetColumn, preserve));
    },
...
// --- Tree view ---
const registerTreeDataProvider = outlineService.registerTreeDataProvider;
const createTreeView = outlineService.createTreeView;
const provideOutlines = outlineService.provideOutlines;
...
module.exports = {
  ...
  get tabGroups() {
    ...
    return {
      all: groups,
      activeTabGroup: groups.find(group => group.isActive) || groups[0] || null,
  ...
  get activeColorTheme() {
    try {
      const snapshot = vscodeThemeCss.buildPulsarThemeSnapshot({ document, window, atom });
      const names = (atom.themes.getActiveThemeNames && atom.themes.getActiveThemeNames()) || [];
      return { kind: snapshot.themeClass === 'vscode-dark' ? 2 : 1, id: names[0] || 'default' };
    } catch (e) {
      const names = (atom.themes.getActiveThemeNames && atom.themes.getActiveThemeNames()) || [];
  ...
  onDidStartTerminalShellExecution: _onDidStartTerminalShellExecution.event,
