...
function provideOutlines() {
  return {
    ...
    async getOutline(editor) {
      ...
      console.log("OUTLINE", outlineTrees)
...
async function treeElementToOutlineTree(provider, element, viewId) {
  ...
  console.log("TREE", treeItem)

  const label = treeItemLabel(treeItem) || String(viewId || 'Tree Item');
  // const icon = treeItemIcon(treeItem);
  const isCollapsed = treeItem.collapsibleState === TreeItemCollapsibleState.Collapsed;
  const outlineTree = {
    plainText: label,
    // representativeName: label,
    // kind: treeItemKind(treeItem),
    // collapsed: isCollapsed,
    // expanded: !isCollapsed,
    startPosition: { row: 0, column: 0 },
    // endPosition: { row: 0, column: 0 },
    // landingPosition: { row: 0, column: 0 },
    children
  };
  if (treeItem?.iconPath?.id) outlineTree.icon = iconFor(treeItem.iconPath.id)
  return outlineTree;
}
